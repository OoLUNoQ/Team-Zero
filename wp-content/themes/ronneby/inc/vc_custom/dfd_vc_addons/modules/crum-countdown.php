		...
		function _dfd_countdown_shortcode( $atts ) {
			...
			if ( is_array( $countdown_opt ) ) {
				foreach ( $countdown_opt as $opt ) {
					...
					if ( 'smonth' === $opt ) {
						$count_frmt .= '<span class="number-wrap ' . $shadow . '" '.$wrap_style.'><span class="number" ' . $number_style . '>%-z</span><span class="period" '.$period_style.'>' . esc_html__( 'Months', 'dfd' ) . '</span><i></i><b></b>'.$dot_html.'</span>';
					}
					if ( 'sweek' === $opt ) {
