				...
				<div class="dfd-product-wrap">
					...
					<div class="dfd-product-top <?php echo $product_style == 'style-1' ? esc_attr($info_alignment) : '' ?>">
						...
						if($enable_cat_tag == 'yes' && !empty($subtitle)) :
							...
							echo '<'.esc_attr($subtitle_options['tag']).' class="dfd-woo-single-subtitle widget-sub-title ' . esc_attr($subtitle_options['class']) . '" ' . $subtitle_options['style'] . '><a href="'. esc_url(get_permalink()) .'">'. esc_html($subtitle) .'</a></'.esc_attr($subtitle_options['tag']).'>';
						endif;
						?>
