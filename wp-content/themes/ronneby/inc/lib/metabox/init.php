...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		global $wp_version, $dfd_ronneby;
		// Only use minified files if SCRIPT_DEBUG is off
		$min = (isset($dfd_ronneby['dev_mode']) && $dfd_ronneby['dev_mode'] == 'on' && defined('DFD_DEBUG_MODE') && DFD_DEBUG_MODE) ? '' : '.min';
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => esc_html__( 'Clear' ,'dfd' ),
					'defaultString' => esc_html__( 'Default', 'dfd' ),
					'pick'          => esc_html__( 'Select Color', 'dfd' ),
					'current'       => esc_html__( 'Current Color', 'dfd' ),
				) );
			}
		} else {
...
//		wp_register_script( 'cmb-scripts', CMB_META_BOX_URL .'js/cmb.js', $scripts, self::CMB_VERSION );
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => esc_html__( 'Select / Deselect All', 'dfd' ),
		) ) );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	if ( $echo ) {
		echo sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'dfd' ) );;
	}

	return sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'dfd' ) );;
}
